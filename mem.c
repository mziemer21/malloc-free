...
void* Mem_Alloc(int size)
{
	...
	/*set when a space was allocated to stop looking*/
	bool allocated = false;
	/*search for a free block that is large enough*/
	while(block != NULL && !allocated)
	{	
		/*check space is free and large enough to be allocated*/
		if(block->size_status%2 !=0 || size > block->size_status)
		{
			/*If current block doesn't work get next block*/
			block = block->next;

		}
		else
		{
			/*Indicate that a free block was found*/
			allocated = true;
			/*block is just the right size no need to seperate the free space*/
			if((block->size_status - size <= (int)sizeof(block_header)) && (block->size_status - size >= 0))
			{
				block->size_status++;

				/*return the free space not the block header*/
				return block+1;
			}
			/*size is less than free space so separate the space*/
			else
			{
				/*find new free address*/
				char* newAddr;
				newAddr = (char*) block + (int)sizeof(block_header) + size;
				
				/*Assign a block_header to the new address*/
				block_header* newBlock;
				newBlock = (block_header*)newAddr;

				/*Set the new block to the correct size_status*/
				newBlock->size_status = block->size_status - (int)sizeof(block_header) - size;

				/*Link the new block into the list*/
				newBlock->next = block->next;
				block->next = newBlock;
  
  				/*Set the used block to the correct size and mark as allocated*/
				block->size_status = size+1;
  
				return block+1;				
			}
		}	
	} 
	/*Could not find a free space*/
	return NULL;
}
...
int Mem_Free(void *ptr)
{
	...
	/*check ptr is valid part of memory*/
	bool valid = false;
	block_header* blk;
	blk = list_head;

	/*loop through memory list*/
	while(blk != NULL && !valid)
	{
	...
	/*find the start of the block_header for the free space being freed*/
	char* addr;
	addr = ptr;
	addr = addr - (int)sizeof(block_header);
	/*Assign a new block_header to that address*/
	block_header* bptr;
	bptr = (block_header*) addr;

	/*check space is valid and allocated*/
  	if ((bptr->size_status & 1) == 0)
	{
		return -1;
	}
	/*set space back to free*/
	bptr->size_status--;
	...
	if(bptr->next != NULL)
	{
		/*check next block and coalesce if possible*/
		if(bptr->next->size_status % 2 == 0)
		{
			bptr->size_status = bptr->size_status + (int)sizeof(block_header) + 
						bptr->next->size_status;
			bptr->next = bptr->next->next;
		}
	}

	/*check if bptr is first in list*/
	if(bptr != list_head)
	{
		/*check with previous block and coalesce if possible*/
		block_header* block;
		block = list_head;

		/*look for block before bptr*/
		while (block->next != bptr)
		{
			block = block->next;	
		}
		...
		{
			block->size_status = block->size_status + (int)sizeof(block_header) +
						bptr->size_status;
